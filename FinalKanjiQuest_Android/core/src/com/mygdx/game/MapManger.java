...
public class MapManger {
    ...
    public void loadMap(String mapName){
        ...
        if(spawnsLayer == null){
            Gdx.app.log(TAG, "No spawns layer!");
        } else {
        ...
        Gdx.app.log(TAG, "Player Start: (" + playerStart.x +
                ", "  + playerStart.y + ")");
    ...
    private void setClosestStartPosition(final  Vector2 position) {
        Gdx.app.log(TAG, "setClosestStartPosition INPUT: (" + position.x + "," + position.y + ") " + currentMapName);
        ...
        for( MapObject object: spawnsLayer.getObjects()){
            if( object.getName().equalsIgnoreCase(PLAYER_START) ){
                ...
                Gdx.app.log(TAG, "distance: " + distance + " for " + currentMapName);

                if( distance < shortestDistance || shortestDistance == 0 ){
                    closestPlayerStartPosition.set(playerStartPositionRect);
                    shortestDistance = distance;
                    Gdx.app.log(TAG, "closest START is: (" + closestPlayerStartPosition.x + ","
